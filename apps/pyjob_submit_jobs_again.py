...
def main():
    ...
    parser.add_argument("type", type=str, help="job type", choices=['ma', 'xy', 'ex'])
    parser.add_argument("rms", type=int, nargs='+', help="rms numbers")
    parser.add_argument("-d", "--directory", type=str, help="trackcpp directory path")
    ...
    curdir = os.getcwd()
    if args.directory and any([args.directory.endswith('trackcpp'), args.directory.endswith('trackcpp/')]):
        ...
        print('Change the current working directory to trackcpp directory or pass its path as argument.')
