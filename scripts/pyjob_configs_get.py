...
def main():
    ...
    parser.add_argument(
        '--showCalendar', dest='sCal', action='store_true',
    ...
    if opts.sCal:
        sortCal = lambda x: (
            getattr(calendar, x[0][0].upper()), x[0][1], x[0][2])
        ...
        for k, v in sorted(ConfigsReceived.items(), key=lambda x: x[0]):
            ...
            for kl in sorted(conj, key=lambda x: x):
                ...
                for day, dados in sorted(table.items(), key=sorTab):
                    ...
                    nums += '{:^10d}'.format(vl)
                print('{:^17s}{:s}'.format(
                    '{0:02d}:{1:02d}'.format(kl[0], kl[1]), nums))
        return
    ...
    for k, v in sorted(ConfigsReceived.items(), key=lambda x: x[0]):
        ...
        print('{key:17s}{val.active!s:^7s}{val.numcpus:^7d}{N:^10d}'
              ...
              '{val.niceness:^6d}'.format(
                  key=k, val=v, N=NumJobs, acc='yes' if v.MoreJobs else 'no'))
        TNCPUs += v.numcpus
        TNAllow += NumJobs
        TJThere += v.totalJobs
        TJRun += v.running
